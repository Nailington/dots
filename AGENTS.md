...
- Commands the user runs are on `PATH` after a switch: `nh`, `sync-age-recipients`, `nixos-remote-install`. Do not tell them to `nix run` those scripts. `nh` is not a Nix subcommand. It is the [nix-community/nh](https://github.com/nix-community/nh) package, installed by this flake, wrapping `nixos-rebuild` and `darwin-rebuild` with shorter commands and other quality-of-life features. The binary on `PATH` is `pkgs/nh-wrapped.nix`.
